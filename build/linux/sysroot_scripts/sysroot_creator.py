...
LOONG64_ARCHIVE_URL = "https://mirrors.loong64.com/debian/"
APT_SOURCES_LIST = [
...
APT_SOURCES_LISTS = {
    ...
    "loong64": APT_SOURCES_LIST_RISCV,
...
def get_archive_url(arch: str) -> str:
    if arch == "loong64":
        return LOONG64_ARCHIVE_URL
    return ARCHIVE_URL


def parse_package_metadata(package_meta: str) -> dict[str, str]:
    fields = {}
    for line in package_meta.splitlines():
        ...
        if not separator:
            raise ValueError(f"Malformed package metadata field: {line}")
...
def generate_package_list_dist_repo(
    ...
    archive_url = get_archive_url(arch)
    ...
    verify_package_listing(
        package_file_arch, package_list, dist, build_dir, archive_url
    )

    with lzma.open(package_list, "rt") as src:
        return [
            parse_package_metadata(package_meta)
